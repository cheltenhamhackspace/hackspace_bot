...
class Quarantine(commands.Cog):
    """
    The Quarantine command takes a stat date and gives the time since
    then as the length of the quarantine
    """
    def __init__(self, bot, q_start):
        """
        Takes bot object and quarentine start date
        
        Args:
            bot (class)
            q_start (string): format="%d/%m/%Y %H:%M:%S"
        """
        self.bot = bot
